...
function assertMaster(context) {
  if (!context.auth || context.auth.token.role !== "master") {
...
exports.setUserRole = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    console.error("Usuário não encontrado ao atribuir papel:", error);
    throw new functions.https.HttpsError(
      "not-found",
  ...
  if (role === "influencer") {
    profileData.influencer = {
      publicName: cleanOptionalString(data?.publicName || profileData.displayName, 120),
      couponCode: cleanOptionalString(data?.couponCode, 40).toUpperCase(),
      commissionPercent: Number.isFinite(Number(data?.commissionPercent))
        ? Math.max(0, Math.min(100, Number(data.commissionPercent)))
        : 0,
      pixKey: cleanOptionalString(data?.pixKey, 160)
    };
  }

  await Promise.all([
    admin.auth().setCustomUserClaims(uid, { role }),
    db.collection("userProfiles").doc(uid).set(profileData, { merge: true })
  ]);

  return {
    uid,
    role,
    message: `Papel ${role} atribuído com sucesso. O usuário deve renovar a sessão para receber a nova claim.`
  };
});

exports.refreshUserRoleToken = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      "unauthenticated",
