...
async function applyCoupon() {
    ...
    // =====================================================
    // REGRA: SEM CUPOM = SEM DESCONTO
    // =====================================================
    const noCoupon =
        !normalizedCode ||
        normalizedCode === "digite seu cupom" ||
        normalizedCode === "informe um código de cupom.";

    if (noCoupon) {
        appliedCoupon = null;

        if (msgEl) {
            msgEl.className = "form-text small text-muted";
    ...
    try {
        ...
        // Erro na validação = SEM DESCONTO
        appliedCoupon = null;
...
// Remove o cupom atualmente aplicado
function removeCoupon() {
    ...
    if (msgEl) {
        msgEl.className = "form-text small text-muted";
        msgEl.innerText = "Cupom removido.";
    }

    // Remove imediatamente qualquer desconto visual.
    if (discountEl) {
        discountEl.innerText = "- R$ 0,00";
    }

    // Recalcula o carrinho sem cupom.
    updateCart();
}
...
// =====================================================
// CÁLCULO FINANCEIRO — ÚNICA FONTE DE VERDADE
// =====================================================
//
// Somente um cupom realmente validado participa
// do cálculo financeiro.
//
// Sem cupom válido:
//   coupon = null
//
// O contrato financeiro calcula:
//   - subtotal
//   - desconto
//   - total
//   - comissão
...
const normalizedCouponText =
    String(couponText)
        .trim()
        .toLowerCase();

const couponIsEmpty =
    !normalizedCouponText ||
    normalizedCouponText === "digite seu cupom" ||
    normalizedCouponText === "informe um código de cupom.";
...
const {
    ...
    const cartSubtotal = financials.subtotal;
const cartDiscount = financials.discount;
const cartTotal = financials.total;
...
// =====================================================
// REGRA FINANCEIRA:
// somente um cupom realmente aplicado pode participar
// do cálculo do pedido.
//
// Se não existe cupom válido, o motor recebe null.
// =====================================================
...
const financials =
   ...
   const orderData = {
    ...
    commission: {
        applicable: Boolean(
    checkoutCoupon && calcCommission > 0
),
    ...
    await createCommissionOperation({
        commission: {
            ...
            code:
                checkoutCoupon.code,
            pixKey:
                checkoutCoupon.pixKey || "Não informada",
            orderTotal:
                finalTotal,
            commissionPercent:
                checkoutCoupon.commissionPercent || 0,
            commissionValue:
                calcCommission,
            payoutStatus:
                COMMISSION_STATUS.PENDENTE
        ...
        actorId: customerId,
        actorName: customerData.name,
        reason: "Comissão criada pelo checkout",
        ...
        if (checkoutCoupon) {
    message += `\n*CUPOM APLICADO:* ${checkoutCoupon.code} (-R$ ${calcDiscount.toFixed(2).replace('.', ',')})`;
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // =====================================================
    // MONITORAMENTO DO CAMPO DE CUPOM
    // =====================================================
    const couponInput =
        document.getElementById("coupon-code");
    ...
    if (couponInput) {
        ...
        couponInput.addEventListener(
            ...
            () => {
                ...
                const noCoupon =
                    !value ||
                    value === "digite seu cupom" ||
                    value === "informe um código de cupom.";

                if (noCoupon) {

                    // Remove imediatamente o cupom anterior.
                    appliedCoupon = null;
